...
/*!
 * Overwrite one user with another.
 *
 * @param dest User to be overwritten.
 * @param src User to copy.
 * @note Call usr_free beforehand. This function doesn't do it.
 */
static void usr_replace(user *dest, user *src) {
	size_t srcidx = src  - USERS->arr;
	size_t dstidx = dest - USERS->arr;
	CONNS->arr[dstidx] = CONNS->arr[srcidx];
	*dest = *src;
}
