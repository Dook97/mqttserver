...
static pkt_handler verify_fixed_header(const pkt_info *pinfo, const user *usr) {
	...
	case CONNECT:
		 ...
		 * [MQTT-3.1.3-5]
		 */
		if (pinfo->flags == CONNECT_DEF_FLAGS && pinfo->rem_len >= 13 && !usr->connect_recieved)
	...
	case PUBLISH:
		 ...
		 * present.
		 */
		if (!(pinfo->flags & 0x0e) && pinfo->rem_len >= 3 && usr->connect_recieved)
	...
	case SUBSCRIBE:
		 ...
		 */
		if (pinfo->flags == SUBSCRIBE_DEF_FLAGS && pinfo->rem_len >= 6 && usr->connect_recieved)
	...
	case UNSUBSCRIBE:
		 ...
		 */
		if (pinfo->flags == UNSUBSCRIBE_DEF_FLAGS && pinfo->rem_len >= 5 && usr->connect_recieved)
			return unsubscribe_handler;
		break;
	case PINGREQ:
		/* see Figure 3.33 - PINGREQ Packet fixed header */
		if (pinfo->flags == PINGREQ_DEF_FLAGS && pinfo->rem_len == 0 && usr->connect_recieved)
			return pingreq_handler;
		break;
	case DISCONNECT:
		/* see Figure 3.35 - DISCONNECT Packet fixed header */
		if (pinfo->flags == DISCONNECT_DEF_FLAGS && pinfo->rem_len == 0
